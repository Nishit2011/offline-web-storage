...
const Order = {
    ...
    loadWS: async() => {
        if(localStorage.getItem("ca-cart")){
            try {
                Order.cart = JSON.parse(localStorage.getItem("ca-cart"))
                Order.render();
            } catch (error) {
    ...
    saveWS: async() => {
        localStorage.setItem("ca-cart",JSON.stringify(Order.cart))
    },
    add: async id => {
